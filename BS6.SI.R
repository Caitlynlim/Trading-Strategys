...
fee <- function(TxnQty, TxnPrice, Symbol) {
  ...
  return(-0.003 * abs(TxnQty * TxnPrice))}
...
#Add signals
add.signal(strategy.st, name = "sigThreshold", arguments = list(column = "rsi.RSI_14",threshold=30,relationship="gt"), label = "RSI_gt_30")
add.signal(strategy.st, name = "sigThreshold", arguments = list(column = "rsi.RSI_14",threshold=70,relationship="lt"), label = "RSI_lt_70")
...
add.signal(strategy.st, name = "sigFormula",
           arguments = list(formula="RSI_gt_30 & macd_gt_0",
...
add.signal(strategy.st, name = "sigFormula",
           arguments = list(formula="RSI_lt_70 & macd_lt_0",
                            cross = FALSE), label = "Short")
...
add.rule(strategy = strategy.st, name="ruleSignal", arguments = list(
  sigcol = "Short", sigval = TRUE, orderqty = 'all', ordertype = "market",TxnFees = "fee", 
  orderside = "long",prefer="Open", replace = FALSE), type = "exit",label='long exit')
